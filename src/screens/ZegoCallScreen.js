...
export default function ZegoCallScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    // Handle missing zegoCallId - generate one if needed
    if (!callID) {
      console.warn("Missing zegoCallId, generating fallback...");
      const fallbackCallId = `fallback_${userID}_${Date.now()}`;
      console.log("Generated fallback call ID:", fallbackCallId);
      // Update the route params
      route.params.callID = fallbackCallId;
    }
    ...
    const paramsToValidate = {
      ...
      userID,
      userName,
      callID: callID || route.params.callID,
    };

    if (!ZegoCloudService.validateCallParams(paramsToValidate)) {
      setError(
        `Invalid call parameters. Missing: ${!callID ? "callID " : ""}${
          !userID ? "userID " : ""
        }${!userName ? "userName" : ""}`
      );
      return;
  ...
  return (
    <View style={styles.container}>
      ...
      <ZegoUIKitPrebuiltCall
        ...
        userName={userName}
        callID={
          callID || route.params.callID || `fallback_${userID}_${Date.now()}`
        }
